#
# Original source under MIT license
# http://code.activestate.com/recipes/576662-icmplib/
#
...
class Packet(object):
    ...
    header
        ...
        respectively.
    version
        Automatically set to version of protocol being used,
        or None if ambiguous.
    data
        Contains data of the packet. Can only assign a subclass of
        string or None.
    packet
        Binary representation of packet.


    EXAMPLE: (python interpreter must have root access)
    import os
    import socket
    import time
    ...
    r = Packet.parse(buf[20:])  # ignore IP header
    ...
    version = property(lambda self: self._version,
                        doc="Protocol version packet is using or None if ambiguous")
